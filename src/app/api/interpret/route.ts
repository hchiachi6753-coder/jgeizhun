import { NextRequest, NextResponse } from 'next/server';
import { GoogleGenerativeAI } from '@google/generative-ai';

// 初始化 Gemini
const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY || '');

// 純紫微斗數論命架構
const SYSTEM_PROMPT = `你是一位資深紫微斗數命理師，專精於紫微斗數星曜解讀。
參考典籍：《紫微斗數全書》、《太微賦》、《骨髓賦》、《斗數準繩》

【語氣風格】
- 直接、敢講、有畫面。以「我看見的」為核心。
- 短句有力、語氣帶呼吸，像真人論命。
- 每段結尾附「命理師金句」。
- 禁止模糊、表面、籠統。每一分析須具星曜依據、心理層意象與可驗證落點。

【核心理念】
「命盤是統計，不是限制。」你的任務不只是解命，而是讓命主「在文字裡看到自己」。當命主被看懂，就會開始改命。

【解讀架構】

## ✨ 命格開場

用一句「開盤金句」定場，瞬間建立臨場感。
例：「這個盤一打開，氣就沉——這不是平凡之命，是藏鋒之格。」
然後以紫微星曜解析命主的內在心理、性格特質與人生主題。

---

## 🎭 命宮主星深度解析

主星特質、廟旺狀態、輔煞星的影響。
命主的核心性格與行為模式。古籍引用（如適用）。
三方四正的星曜交互影響。

---

## 💼 事業與官祿

官祿宮星曜配置、適合的職業方向、發展模式。
事業上的機會與風險、趨吉避凶建議。
命宮與官祿宮的星曜連動解讀。

---

## 💰 財運與財帛

財帛宮星曜配置、財富累積模式（正財/偏財）。
理財建議與風險提醒。
財帛宮與福德宮的交互影響。

---

## ❤️ 感情與婚姻

夫妻宮星曜配置、感情模式、擇偶傾向。
婚姻中的課題與成長點、實用建議。
桃花星與煞星的影響分析。

---

## 🩺 健康提醒

疾厄宮星曜配置、需注意的身體部位、養生建議。
各星曜對應的健康課題。

---

## 📅 近期流年提醒

當前大限宮位與星曜影響。
今年流年的機會與風險。
具體行動建議與時機把握。

---

## 👥 貴人與小人

命盤中的貴人線索（左輔、右弼、天魁、天鉞等）。
需提防的小人特徵（煞星配置暗示）。
人際相處建議。

---

## 🏁 結語

以一句有力的金句收尾，給命主的核心提醒。
綜合命盤特質給出人生方向建議。

【重要提醒】
1. 每個分析都要有具體的星曜依據
2. 用白話解釋術語，讓一般人能懂
3. 給出可落地執行的建議
4. 語氣要有溫度，像真人在對話
5. 善用古籍金句增添說服力

【排版規則】⚠️ 嚴格遵守，這是美感的關鍵！

設計原則：美美的、看得清楚、層次分明、有結構

1. **章節標題**：用 ## + emoji 開頭（如 ## ✨ 命格開場）
2. **章節分隔**：每個章節之間必須用 --- 分隔線
3. **標題後空行**：每個 ## 後面空一行再寫內容
4. **段落呼吸**：段落之間要空一行，讓閱讀有節奏
5. **金句格式**：命理師金句獨立一行，用「」包起來
6. **條列建議**：建議事項用 → 開頭，條列清晰
7. **emoji 限制**：只有 ## 標題用 emoji，內文完全不用 emoji
8. **重點標示**：星曜名稱用 **粗體** 標示

範例格式：
---
## ✨ 命格開場

這個盤一打開，紫微星坐命...（內容不用 emoji）

「你不是沒有才華，是才華太大，舞台還沒準備好。」

---
## 🎭 命宮主星深度解析

**紫微星** 為北斗主星...

→ 適合領導型工作
→ 需要被尊重的感覺
→ 容易因為驕傲而錯失機會`;

export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { chart } = body;

    if (!chart) {
      return NextResponse.json(
        { error: '缺少命盤資料' },
        { status: 400 }
      );
    }

    // 組織紫微命盤資訊
    const ziweiInfo = formatChartInfo(chart);

    // 計算當前年份和命主年齡
    const currentYear = new Date().getFullYear();
    const birthYear = chart.solarDate.year;
    const age = currentYear - birthYear;

    const prompt = `${SYSTEM_PROMPT}

【重要時間資訊】
- 當前年份：${currentYear}年
- 命主出生年：${birthYear}年
- 命主現年：${age}歲

【紫微斗數命盤】
${ziweiInfo}

請根據以上紫微斗數命盤資料，提供完整的論命解讀。
記住：
1. 當前是${currentYear}年，流年分析要用${currentYear}年
2. 以星曜特質為核心，深入解析命主的內在心理與人生課題
3. 命主現年${age}歲，分析要符合這個人生階段
4. 每個宮位分析都要連結三方四正的星曜配置`;

    // Pro 優先，額度用完自動切 Flash
    let text: string;
    let usedModel: string;
    
    try {
      // 先嘗試 Pro
      const proModel = genAI.getGenerativeModel({ model: 'gemini-1.5-pro' });
      const result = await proModel.generateContent(prompt);
      const response = await result.response;
      text = response.text();
      usedModel = 'pro';
    } catch (proError: any) {
      // Pro 失敗（可能是額度用完），改用 Flash
      console.log('Pro 額度用完，切換到 Flash:', proError.message);
      const flashModel = genAI.getGenerativeModel({ model: 'gemini-2.0-flash' });
      const result = await flashModel.generateContent(prompt);
      const response = await result.response;
      text = response.text();
      usedModel = 'flash';
    }

    return NextResponse.json({
      success: true,
      interpretation: text,
      model: usedModel, // 讓前端知道用了哪個模型
    });

  } catch (error) {
    console.error('Gemini API error:', error);
    return NextResponse.json(
      { error: '解讀生成失敗，請稍後再試' },
      { status: 500 }
    );
  }
}

// 格式化命盤資訊
function formatChartInfo(chart: any): string {
  const lines: string[] = [];

  // 基本資訊
  lines.push('【基本資訊】');
  lines.push(`性別：${chart.gender === 'male' ? '男' : '女'}`);
  lines.push(`農曆：${chart.lunarDate?.yearGanZhi}年 ${chart.lunarDate?.month}月 ${chart.lunarDate?.day}日`);
  lines.push(`五行局：${chart.wuXingJu?.name}`);
  lines.push(`命宮：${chart.mingGong?.gan}${chart.mingGong?.zhi}`);
  lines.push(`身宮：${chart.shenGong?.gongName}`);
  lines.push('');

  // 四化
  if (chart.siHua) {
    lines.push('【四化飛星】');
    lines.push(`化祿：${chart.siHua.lu?.star}（${chart.siHua.lu?.gongName}）`);
    lines.push(`化權：${chart.siHua.quan?.star}（${chart.siHua.quan?.gongName}）`);
    lines.push(`化科：${chart.siHua.ke?.star}（${chart.siHua.ke?.gongName}）`);
    lines.push(`化忌：${chart.siHua.ji?.star}（${chart.siHua.ji?.gongName}）`);
    lines.push('');
  }

  // 十二宮
  lines.push('【十二宮配置】');
  if (chart.gongs) {
    for (const gong of chart.gongs) {
      const mainStars = gong.mainStars?.map((s: any) => {
        let name = s.name;
        if (s.brightness) name += `(${s.brightness})`;
        if (s.siHua) name += s.siHua;
        return name;
      }).join('、') || '無主星';

      const assistStars = gong.assistStars?.map((s: any) => s.name).join('、') || '';
      const shaStars = gong.shaStars?.map((s: any) => s.name).join('、') || '';

      let starInfo = mainStars;
      if (assistStars) starInfo += ` / ${assistStars}`;
      if (shaStars) starInfo += ` / ${shaStars}`;

      const shenGongMark = gong.isShenGong ? ' 【身宮】' : '';
      lines.push(`${gong.name}（${gong.gan}${gong.zhi}）${shenGongMark}：${starInfo}`);
    }
  }

  // 大限
  if (chart.daxian) {
    lines.push('');
    lines.push('【大限運程】');
    lines.push(`起運歲數：${chart.daxian.startAge}歲`);
    lines.push(`運行方向：${chart.daxian.direction}`);
    if (chart.daxian.periods) {
      const periods = chart.daxian.periods.slice(0, 6);
      for (const p of periods) {
        lines.push(`${p.startAge}-${p.endAge}歲：${p.gongName}（${p.ganZhi}）`);
      }
    }
  }

  return lines.join('\n');
}
